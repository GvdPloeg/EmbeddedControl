...
void Read_EEPROM(void){
	...
	U8 data_received[ 4 ] = {0};
	
	for(int j = 1 ; j < 5 ; j++){
		
		packet_EEPROM_received.chip = 0x57 ;
		packet_EEPROM_received.addr_length = 3;
		packet_EEPROM_received.length = 4;
		packet_EEPROM_received.addr[0] = decimal_to_bcd(j);
		...
		for(int i = 0; i < 4 ; i ++){
			switch(i){
				case 0:
				time_uitgang[j] = data_received[i] * 60 * 60 * 60;
				break;
				case 1:
				time_uitgang[j] = time_uitgang[j] + data_received[i] * 60 * 60;
				break;
				case 2:
				time_uitgang[j] = time_uitgang[j] + data_received[i] * 60;
				break;
				case 3:
				time_uitgang[j] = time_uitgang[j] + data_received[i];
				break;
			}
		}
...
void Write_EEPROM(void){
	...
	U8 write_pattern[] =  { ((time_uitgang[output_nmb] / 216000) % 24), ((time_uitgang[output_nmb] / 3600) % 60), ((time_uitgang[output_nmb] / 60) % 60), (time_uitgang[output_nmb] % 60) };
