...
void display_cases(void){
	switch(display_number){
		case 1:
			set_display(15,1,"(OFF)");
			if (joy_button == 1){ set_display(15,1,"(ON)-"); }
				
			set_display(7,2,itoa(hr_timer / 10 % 10));
			set_display(8,2,itoa(hr_timer % 10));
			set_display(10,2,itoa(min_timer / 10 % 10));
			set_display(11,2,itoa(min_timer % 10));
			set_display(13,2,itoa(sec_timer / 10 % 10));
			set_display(14,2,itoa(sec_timer % 10));
				
			set_display(7,3,itoa(bcd_to_decimal(tbuffer[3])));				// Show Number of the Day (between 1 and 7)
			set_display(9,3,itoa((bcd_to_decimal(tbuffer[4])) / 10 % 10));	// Show Day			
			set_display(10,3,itoa((bcd_to_decimal(tbuffer[4])) % 10));		// Show Day				
			Number_to_Month(bcd_to_decimal(tbuffer[5]));					// Show Month
			set_display(18,3,itoa((bcd_to_decimal(tbuffer[6])) / 10 % 10));	// Show Year
			set_display(19,3,itoa((bcd_to_decimal(tbuffer[6])) % 10));		// Show Year
			Number_to_DAY(bcd_to_decimal(tbuffer[3]));					// Show the name of the day
			if(joy_button== 1){ Write_RTC(); }
		break;
		case 2:
			set_time();
			if(joy_button == 1){ set_display(2,1,"Change Alarm  (ON) "); Write_EEPROM();}
			else{ set_display(2,1,"Change Alarm  (OFF)"); }
		break;
	}
}
	...
	switch(display_number){
		...
		case 3:				
			...
			for(int pos = 0 ; pos < 4 ; pos++){
				...
				set_display(16,(pos+1),itoa(time_uitgang_ms[pos] % 1000 / 100 % 10));
				set_display(17,(pos+1),itoa(time_uitgang_ms[pos] % 1000 / 10 % 10));
				set_display(18,(pos+1),itoa(time_uitgang_ms[pos] % 1000 % 10));
			}
		break;
	}
}
...
void set_time(void){	
	int time_ms3 = time_uitgang_ms[output_nmb] % 1000 % 10;
	int time_ms2 = time_uitgang_ms[output_nmb] % 1000 / 10 % 10;
	int time_ms1 = time_uitgang_ms[output_nmb] % 1000 / 100 % 10;
	...
	set_display(16,3,itoa(time_uitgang_ms[output_nmb] % 1000 / 100 % 10));
	set_display(17,3,itoa(time_uitgang_ms[output_nmb] % 1000 / 10 % 10));
	set_display(18,3,itoa(time_uitgang_ms[output_nmb] % 1000 % 10));
	...
	if(joy_button == 1 && y == 3 && x == 3){ output_nmb = (Potmeter_value()/3%4); }
		...
		time_uitgang[output_nmb] = (time_h1 * 10 + time_h2) * 60 * 60 * 1000 + (time_m1 * 10 + time_m2) * 60 * 1000 + (time_s1 * 10 + time_s2) * 1000;
		time_uitgang_ms[output_nmb] = (time_ms1 * 100 + time_ms2 * 10 + time_ms3);
	}
}
