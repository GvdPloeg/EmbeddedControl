...
void Read_EEPROM(void){
	...
	for(int output = 0 ; output < 4 ; output++){
		...
		time_uitgang[output] = data_received[0] * 60 * 60 * 1000 + data_received[1] * 60 * 1000 + data_received[2] * 1000;
		time_uitgang_ms[output] = data_received[3] * 100 + data_received[4];
...
void Write_EEPROM(void){
	...
	U8 write_pattern[] =  { ((time_uitgang[output_nmb] / 3600000) % 24), ((time_uitgang[output_nmb] / 60000) % 60), ((time_uitgang[output_nmb] / 1000) % 60), (time_uitgang_ms[output_nmb] % 1000 / 100), (time_uitgang_ms[output_nmb] % 1000 % 100) };
...
void Write_RTC(void){
	...
	int time_h = hr_timer / 10 % 10;
	int time_hh = hr_timer % 10;
	int time_m = min_timer / 10 % 10;
	int time_mm = min_timer % 10;
	int time_s = sec_timer / 10 % 10;
	int time_ss = sec_timer % 10;
	...
	if(joy_button == 1 && y == 2){
		...
		hr_timer = time_h * 10 + time_hh;
		min_timer = time_m * 10 + time_mm;
		sec_timer = time_s * 10 + time_ss;
	}
